// Custom libmpv stream protocol ("ytn://") that fetches googlevideo media via
...
// Register the "ytn" protocol on this mpv handle (call once, after init).
void register_stream(mpv_handle* mpv);

// Wrap a real https URL + user-agent into a ytn:// URI for loadfile/audio-add.
